 ...
 *
 *          the audiocontroller take care that the audio on the audio-device is
 *          close to the masterclock it's the server side technic to synchronize
 *          the video and audio
 *
 */
public class AudioController extends Html5Controller {
	...
	public void followMasterClock(String name) {
		...
		if (clock.running()) {
			...
			if (audionode != null) {
				...
				JSONObject audionodeobject = audionode.toJSONObject("en",
						"title,location,date,year,img,mp3,description,title-en,location-en,date-en,year-en,mp3-en,description-en,img-en,mp4,autoplay,controls,videolist,selected,wantedtime");
			...
			if (audionode != null) {
				...
				JSONObject audionodeobject = audionode.toJSONObject("en",
						"title,location,date,year,mp3,img,img-en,description,title-en,location-en,date-en,year-en,mp3-en,description-en,mp4,autoplay,controls,videolist,selected,wantedtime");
				System.out.println("VideoNodeObject: " + audionodeobject.toJSONString());
	...
	/**
	 * Brings the audioplayer to the screen of the audiodevice
	 */
	...
	public void onClockUpdate(String path, FsNode node) {
		...
		if (clock != null) {
			if (id.equals(clock.getClockName())) { // extra check since mem
													// track model not 100%
													// done.
	...
	/**
	 * By starting or stopping a video, we need the new audio data
	 * 
	 * @param path
	 * @param node
	 */
	public void onVideoUpdate(String path, FsNode node) {
		...
		if (node != null) {
			// System.out.println("node: " + node.asXML() + "action-Prop: " +
			// node.getProperty("action"));
			if ((String) node.getProperty("action") != null) {
				...
				if (action.equals("startvideo")) {
					...
					if (node != null) {
						...
						if (audionode != null) {
							...
							// FsNode snode =
							// Fs.getNode("/domain/senso/user/rbb/collection/homepage/video/"+requestedtVideo);
							FsNode snode = Fs.getNode("/domain/senso/tmp/videocontrollerapp/video/1");
							...
							JSONObject audionodeobject = audionode.toJSONObject("en",
									"img,title,location,date,year,mp3,description,img-en,title-en,location-en,date-en,year-en,mp3-en,description-en,mp4,autoplay,controls,videolist,selected,wantedtime");
							System.out.println("VideoNodeObject: " + audionodeobject.toJSONString());
				...
				if (action.equals("closevideo")) {
					...
					Fs.setProperty("/domain/senso/tmp/videocontrollerapp/video/1", "mp3", "");
					Fs.setProperty("/domain/senso/tmp/videocontrollerapp/video/1", "mp3-en", "");
					Fs.setProperty("/domain/senso/tmp/videocontrollerapp/video/1", "img",
							"http://rbb.noterik.com/eddie/apps/videoremote/img/img_ger.jpg");
					Fs.setProperty("/domain/senso/tmp/videocontrollerapp/video/1", "img-en",
							"http://rbb.noterik.com/eddie/apps/videoremote/img/img_gb.jpg");
					...
					if (node != null) {
						...
						if (audionode != null) {
							...
							JSONObject audionodeobject = audionode.toJSONObject("en",
									"title,location,date,year,img,img-en,mp3,description,title-en,location-en,date-en,year-en,mp3-en,description-en,mp4,autoplay,controls,videolist,selected,wantedtime");
							System.out.println("VideoNodeObject: " + audionodeobject.toJSONString());
